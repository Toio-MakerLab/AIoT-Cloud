...
export class ApiConfigService {
	...
	get kafkaConfig() {
		const saslEnabled =
			this.configService.get<string>("KAFKA_SASL_ENABLED", "false") === "true";

		return {
			brokers: this.getString("KAFKA_BROKERS"),
			clientId:
				this.configService.get<string>("KAFKA_CLIENT_ID") ?? "aiot-lab-service",
			groupId:
				this.configService.get<string>("KAFKA_GROUP_ID") ??
				"aiot-lab-service-consumer",
			ssl:
