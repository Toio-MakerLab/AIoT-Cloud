    ...
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="scale-95 rounded-full">
          ...
          <span className="sr-only">{t('themeSwitch.toggleTheme')}</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => setTheme(Theme.Light)}>
          {t('commandMenu.light')} <IconCheck size={14} className={cn('ml-auto', theme !== Theme.Light && 'hidden')} />
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme(Theme.Dark)}>
          {t('commandMenu.dark')}
          <IconCheck size={14} className={cn('ml-auto', theme !== Theme.Dark && 'hidden')} />
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme(Theme.System)}>
          {t('commandMenu.system')}
          <IconCheck size={14} className={cn('ml-auto', theme !== Theme.System && 'hidden')} />
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
