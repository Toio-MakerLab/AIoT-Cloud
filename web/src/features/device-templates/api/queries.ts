...
export const useCreateDeviceTemplateMutation = () => {
	...
	return useMutation({
		mutationFn: (data: ICreateDeviceTemplate) =>
			deviceTemplatesApi.createDeviceTemplate(data),
		onSuccess: () =>
			queryClient.invalidateQueries({ queryKey: [DEVICE_TEMPLATES_QUERY_KEY] }),
	});
};
...
export const useUpdateDeviceTemplateMutation = () => {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: ({ id, data }: { id: string; data: IUpdateDeviceTemplate }) =>
			deviceTemplatesApi.updateDeviceTemplate(id, data),
		onSuccess: () =>
			queryClient.invalidateQueries({ queryKey: [DEVICE_TEMPLATES_QUERY_KEY] }),
	});
};
