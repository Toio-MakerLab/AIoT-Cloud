...
export function AddDeviceDialog({
	...
	const [registeredDeviceId, setRegisteredDeviceId] = useState<string | null>(
		null,
	);
				...
				{step === "details" ? (
					<>
						<DialogHeader>
							...
							<DialogDescription>
								Choose a template and name your device, then scan its QR code.
							</DialogDescription>
						</DialogHeader>
						<Form {...form}>
							<form
								id="device-details-form"
					...
					<>
						<DialogHeader>
							...
							<DialogDescription>
								Scan the QR code printed on the physical device, or type its ID
								manually.
							</DialogDescription>
						</DialogHeader>
						...
						<div className="space-y-4">
							...
							{scanError && useCamera ? (
								<p className="text-destructive text-sm">
									{scanError} Use manual entry below instead.
								</p>
							) : null}

							<div className="space-y-2">
								<Label htmlFor="manual-device-id">
									Device ID (manual entry)
								</Label>
								<div className="flex gap-2">
									...
									<Button
										type="button"
										disabled={
											!manualDeviceId.trim() || registerDevice.isPending
										}
					...
					<>
						<DialogHeader>
							...
							<DialogDescription>
								Set the network parameters this device will fetch at boot, or
								skip and configure later.
							</DialogDescription>
						</DialogHeader>
						<Form {...configForm}>
							<form
								id="device-add-config-form"
