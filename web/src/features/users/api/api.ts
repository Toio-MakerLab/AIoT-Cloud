...
import type { ICreateUser, IPageDto, IResponseCore, IUpdateUser, IUser, IUsersQueryParams } from './types';
...
export const usersApi = {
  ...
  updateUser: async (id: string, data: IUpdateUser) => {
    const response = await apiClient.patch<IResponseCore<IUser>>(`/users/${id}`, data);
    return unwrap(response.data);
  },
};
