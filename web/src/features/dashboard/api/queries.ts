...
export const useCreateDashboardMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [DASHBOARDS_QUERY_KEY] }),
  });
};
...
export const useUpdateDashboardMutation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: ISaveDashboard }) => dashboardApi.updateDashboard(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [DASHBOARDS_QUERY_KEY] }),
  });
};
...
export const useTriggerDeviceActionMutation = (deviceId: string) =>
  useMutation({
    mutationFn: (data: ITriggerDeviceAction) => deviceApi.triggerDeviceAction(deviceId, data),
  });
