...
export function useDeviceSocket(deviceIds: string[]) {
	...
	const [latestByDevice, setLatestByDevice] = useState<Map<string, ILatestTelemetry>>(
		new Map(),
	);
	const [historyByDevice, setHistoryByDevice] = useState<Map<string, ITelemetryPoint[]>>(
		new Map(),
	);
	...
	useEffect(() => {
		...
		const handleTelemetry = (event: TelemetryEventPayload) => {
			...
			setHistoryByDevice((prev) => {
				...
				const appended = [
					...existing,
					{ payload: event.payload, recordedAt: event.recordedAt },
				];
				next.set(
					event.deviceId,
	...
	const seedHistory = (deviceId: string, points: ITelemetryPoint[]) => {
		setHistoryByDevice((prev) => {
			...
			next.set(
				deviceId,
				points.length > TELEMETRY_HISTORY_LIMIT
					? points.slice(points.length - TELEMETRY_HISTORY_LIMIT)
					: points,
			);
			return next;
		});
