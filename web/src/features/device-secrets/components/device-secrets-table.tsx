...
export function DeviceSecretsTable({ data }: Props) {
  ...
  if (data.length === 0) {
    return <p className="text-muted-foreground py-8 text-center text-sm">No device secrets yet. Create one to let device firmware authenticate.</p>;
  }
        ...
        {data.map((secret) => {
          ...
          return (
            <TableRow key={secret.id}>
              <TableCell className="font-medium">{secret.label || <span className="text-muted-foreground">Untitled</span>}</TableCell>
              <TableCell>
                <Badge variant={isRevoked ? 'outline' : 'default'}>{isRevoked ? 'Revoked' : 'Active'}</Badge>
              </TableCell>
