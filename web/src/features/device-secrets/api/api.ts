...
export const deviceSecretsApi = {
	getDeviceSecrets: async () => {
		const response =
			await apiClient.get<IResponseCore<IDeviceSecret[]>>("/device-secrets");
		return unwrap(response.data);
	},
	createDeviceSecret: async (data: ICreateDeviceSecret) => {
		const response = await apiClient.post<IResponseCore<ICreatedDeviceSecret>>(
			"/device-secrets",
			data,
		);
		return unwrap(response.data);
	...
	revokeDeviceSecret: async (id: string) => {
		const response = await apiClient.post<IResponseCore<IDeviceSecret>>(
			`/device-secrets/${id}/revoke`,
		);
		return unwrap(response.data);
	},
};
